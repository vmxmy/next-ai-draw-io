...
系统采用分级配额管理，不同用户等级享有不同的使用限制：

| 等级 | 日请求数 | 日 Token 数 | TPM (Token/分钟) | 说明 |
|:---:|:---:|:---:|:---:|:---|
| **匿名用户** | 20 | 200,000 | 30,000 | 未登录，基于 IP 限流 |
| **免费版** | 30 | 500,000 | 50,000 | 已登录用户默认等级 |
| **专业版** | 100 | 1,000,000 | 100,000 | 付费用户，深度使用 |
| **企业版** | ∞ | ∞ | ∞ | 无限制，团队/企业用户 |

> 配额每日 UTC 00:00 重置。TPM 限制为滑动窗口（60秒）。
...
| 模式 | 配额限制 | API 费用 | 适用场景 |
|:---:|:---:|:---:|:---|
| **系统默认** | 受等级限制 | 平台承担 | 轻度使用、体验功能 |
| **BYOK** | 无限制 | 用户自理 | 重度使用、专业需求 |

#### Fast vs Max 模型选项

用户可在界面切换两种模型模式，适应不同场景：
...
顶部工具栏的配额图标会根据使用情况动态变色，帮助用户快速了解配额状态：

| 颜色 | 状态 | 说明 |
|:---:|:---:|:---|
| 🟢 **绿色** | BYOK 模式 | 使用自己的 API Key，无配额限制 |
| ⚪ **灰色** | 正常 | 配额使用率 < 80%，可正常使用 |
| 🟡 **黄色** | 警告 | 配额使用率 ≥ 80%，建议节省使用 |
| 🔴 **红色** | 超限 | 配额使用率 ≥ 100%，已达上限 |

> 点击配额图标可查看详细的配额使用情况（日请求数、日 Token 数、TPM）。

#### 一键应用主题样式

输入框旁的「应用主题」按钮（🎨）可将当前 UI 主题的配色方案一键应用到图表中：

- **功能**：自动将当前主题的主色调、背景色、边框色等应用到图表元素
- **场景**：快速统一图表与界面的视觉风格，无需手动调整
- **操作**：点击按钮后，AI 会分析当前主题并更新图表样式
